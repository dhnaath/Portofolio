...
const ujianKhususCourses: string[] = [
        ...
        <table className="w-full text-left border-collapse table-fixed min-w-[800px] font-serif">
          ...
          <tbody className="divide-y divide-gray-200 bg-[#FFFFFF]">
            {sortedCourses.map((course, index) => {
              ...
              const isUjianKhusus = ujianKhususCourses.includes(course.mataKuliah);
                ...
                >
                  <td className={`py-3 px-4 text-center ${
                    isHighlighted 
                      ...
                      : (isUjianKhusus || isRepeated)
                        ? 'bg-white group-hover:bg-[#F4F3F0] text-[#5B6572]'
                        : 'text-[#5B6572]'
                  }`}>{index + 1}</td>
                  <td className={`py-3 px-4 ${(isHighlighted || isRepeated || isUjianKhusus) ? 'text-[#222222] font-medium truncate' : 'text-[#222222] truncate'}`}>{course.mataKuliah}</td>
